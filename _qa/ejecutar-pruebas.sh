...
# Requiere el servidor local en el puerto 8123 (o BASE=… para producción, ver proxy-produccion.py). Uso: _qa/ejecutar-pruebas.sh [--movimiento-reducido]
CH="/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"
EXTRA=""; [ "${1:-}" = "--movimiento-reducido" ] && EXTRA="--force-prefers-reduced-motion"
"$CH" --headless=new --disable-gpu $EXTRA --window-size=1280,900 --virtual-time-budget=240000 --dump-dom "${BASE:-http://127.0.0.1:8123}/_qa/pruebas.html" 2>/dev/null > /tmp/pv-pruebas.html
python3 - <<'PY'
import re, html
